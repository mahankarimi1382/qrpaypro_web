...
@section('content')
    ...
    <section class="checkout-section bg_img" data-background="{{ get_image('frontend/') }}/images/checkout/checkout-bg-1.jpg">
        <div class="container">
                ...
                <div class="col-lg-6">
                    <div class="checkout-wrapper">
                        <div class="checkout-top-area">
                            ...
                            <span
                                class="price-area">{{ get_amount($request_record->amount, $request_record->currency, 4) }}</span>
                        </div>
                        ...
                        <form class="checkout-form" method="POST" action="{{ $submit_form_url }}" id="payment-form">
                            ...
                            <div class="radio-wrapper">
                                @if (@$gateway_setting->wallet_status == true)
                                    <div class="radio-item">
                                        ...
                                        @endphp
                                        <span
                                            class="amount-area">{{ get_amount($request_record->amount, $request_record->currency, 4) }}</span>
                                    </div>
                                @endif
                                @if (@$gateway_setting->virtual_card_status == true)
                                    <div class="radio-item">
                                        ...
                                        @if (isset($user->payment_type))
                                            ...
                                            @endphp
                                            <span
                                                class="amount-area">{{ get_amount($request_record->amount, $request_record->currency, 3) }}</span>
                                        @else
                                            @php
                                            ...
                                            @else
                                                <span
                                                    class="amount-area">{{ get_amount($payable_card_balane, $card->currency, 4) }}</span>
                                            @endif
                                        @endif
                            ...
                            @if (
                                @$gateway_setting->wallet_status === false &&
                                    @$gateway_setting->virtual_card_status === false &&
                                    @$gateway_setting->master_visa_status === false)
                                <div class="btn-area">
                                    <button type="button" class="btn--base w-100"><i
                                            class="las la-angle-left ms-1"></i>{{ __('All Payment Option Diabled') }}</button>
                                </div>
                            @else
                                <div class="btn-area">
                            ...
                            @endif
                            <a href="{{ url($request_record->data->cancel_url . '?token=' . $token) }}"
        ...
        <div class="bottom-area">
            <div class="bottom-wrapper">
                <p>{{ __(@$footer->value->language->$lang->footer_text) }} <a class="fw-bold"
                        href="{{ setRoute('index') }}"> {{ $basic_settings->site_name }}</a></p>
                <ul class="bottom-area-list">
                    @foreach ($policies ?? [] as $key => $data)
                        <li><a href="{{ setRoute('useful.link', $data->slug) }}"
                                target="_blank">{{ @$data->title->language->$lang->title }}</a></li>
                    @endforeach
                </ul>
            </div>
        </div>
