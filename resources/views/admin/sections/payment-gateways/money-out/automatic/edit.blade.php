...
@section('content')
    <form action="{{ setRoute('admin.payment.gateway.update',['withdraw','automatic',$gateway->alias]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method("PUT")
...
@push('script')
    <script>
        ...
        function setInputFieldsName(firstGatewayCurrencyItem,currency){
            ...
            firstGatewayCurrencyItem.find(".daily-limit").attr("name",generateInputName(currency,"daily_limit"));
            firstGatewayCurrencyItem.find(".monthly-limit").attr("name",generateInputName(currency,"monthly_limit"));
        }
