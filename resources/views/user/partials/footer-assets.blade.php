 ...
 <script type="module">
     import {
         fileHolderSettings
     } from "{{ asset('frontend/js/fileholder-settings.js') }}";
     import {
         previewFunctions
     } from "{{ asset('frontend/js/fileholder-script.js') }}";
     ...
     function getAllCountries(hitUrl, targetElement = $(".country-select"), errorElement = $(".country-select").siblings(
         ...
         $.post(hitUrl, data, function() {
             ...
             $.each(response, function(index, item) {
                 options +=
                     `<option value="${item.name}" data-id="${item.id}" data-iso2="${item.iso2}" data-mobile-code="${item.mobile_code}" ${selected_old_data == item.name ? "selected" : ""}>${item.name}</option>`;
             });
