                    ...
                    <div class="dash-payment-item-wrapper">
                        <div class="dash-payment-item active">
                            ...
                            <div class="dash-payment-body">
                                <form class="card-form" action="{{ setRoute('user.cardyfie.virtual.card.deposit') }}" method="POST">
                                    @csrf
                                    <div class="row">
                                        ...
                                        <div class="col-xl-12 col-lg-12 form-group">
                                            <label>{{ __("Deposit Amount") }} <span class="text--base">*</span></label>
                                            <div class="input-group">
                                                <input type="text" class="form--control number-input" required placeholder="{{__('enter Amount')}}" name="deposit_amount" value="{{ old("deposit_amount") }}" autocomplete="off">
                                        ...
                                        @if($basic_settings->user_pin_verification == true)
                                                <div class="col-xl-12 col-lg-12">
                                                    <a href="javascript:void(0)" class="btn--base w-100 btn-loading fundBtn" data-bs-toggle="modal" data-bs-target="#checkPin">{{ __("Deposit Card") }} <i class="las la-plus-circle ms-1"></i></a>
                                                </div>
                                            </div>
                                            @include('user.components.modal.pin-check')
                                        @else
                                                <div class="col-xl-12 col-lg-12">
                                                    <button type="submit" class="btn--base w-100 fundBtn btn-loading">{{__("Deposit Card")}} <i class="las la-plus-circle ms-1"></i></button>
                                                </div>
                                            </div>
                                        @endif
                                </form>
...
<script>
    ...
    $("input[name=deposit_amount]").keyup(function(){
        getFees();
        getPreview();
        get_remaining_limits();
    });
    ...
    $("input[name=deposit_amount]").focusout(function(){
        enterLimit();
        get_remaining_limits();
    });
    ...
    function acceptVar() {
        ...
        var currencyFixedCharge   = "{{getAmount($cardReloadCharge->card_deposit_fixed_fee)}}";
        var currencyPercentCharge = "{{getAmount($cardReloadCharge->card_deposit_percent_fee)}}";
    ...
    function feesCalculation() {
        ...
        var sender_amount = $("input[name=deposit_amount]").val();
    ...
    function getPreview() {
        var exchange_rate = getExchangeRate();
        var senderAmount = $("input[name=deposit_amount]").val();
    ...
    function enterLimit(){
        ...
        var sender_amount = parseFloat($("input[name=deposit_amount]").val());
