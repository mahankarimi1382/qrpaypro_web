        ...
        <div class="dashboard-header-wrapper">
            ...
            @if($cardApi->card_limit > 0)
                @if ($customer_card  < $card_limit )
                    <a href="{{ setRoute('user.cardyfie.virtual.card.create') }}" class="btn--base small" >{{ __("create Card") }} <i class="las la-plus ms-1"></i></a>
                @endif
            @else
                <a href="{{ setRoute('user.cardyfie.virtual.card.create') }}" class="btn--base small" >{{ __("create Card") }} <i class="las la-plus ms-1"></i></a>
            @endif
                            ...
                            <div class="dash-payment-item-wrapper">
                                <div class="dash-payment-item active">
                                    <div class="card-header-btn-wrapper d-flex align-items-center justify-content-between">
                                        <div class="dash-payment-title-area">
                                            ...
                                            @if($cardApi->card_limit > 0)
                                                ...
                                                @else
                                                <h5 class="title">{{ __("My Card") . " (" . ($index+1) . "/" . count($myCards) . ")" }}</h5>
                                            @endif
                                        </div>
                                            ...
                                            <div class="card-custom">
                                                <div class="flip">
                                                    <div class="front bg_img" data-background="{{ get_image(@$cardApi->image ,'card-api') }}">
                                                        ...
                                                        @if ($myCard->masked_pan)
                                                            @php
                                                                $card_pan = str_split($myCard->masked_pan, 4);
                                                            @endphp
                                                        ...
                                                        @endif
                                                        <div class="end">
                                                            <span class="end-text">{{ __("exp. end:") }}</span> <span class="end-date"> {{  $myCard->card_exp_time ?? 'mm/yyyy' }}</span>
                                                        </div>
                                    ...
                                    <div class="virtual-card-btn-area text-center pt-20">
                                        <a href="{{ @$myCard != null ? setRoute('user.cardyfie.virtual.card.details',$myCard->ulid) : 'javascript:void(0)'  }}" class="btn--base"><i class="fas fa-info-circle me-1"></i>{{ __("Details") }}</a>
                                        ...
                                        <a href="{{ setRoute('user.cardyfie.virtual.card.deposit.page',$myCard->ulid) }}" class="btn--base"><i class="fas fa-hand-holding-usd me-1"></i> {{ __("deposit") }}</a>
                                        <a href="{{ setRoute('user.cardyfie.virtual.card.withdraw.page',$myCard->ulid) }}" class="btn--base"><i class="fas fa-hand-holding-usd me-1"></i> {{ __("withdraw") }}</a>
                                        <a href="{{ @$myCard != null ? setRoute('user.cardyfie.virtual.card.transaction',$myCard->ulid) : 'javascript:void(0)'  }}" class="btn--base"><i class="fas fa-arrows-alt-h me-1"></i>{{ __("Transactions") }}</a>

                                        @if($myCard->status != global_const()::CARD_CLOSED_STATUS)
                                            <a href="javascript:void(0)" class="btn--danger close-card" data-id="{{ $myCard->id }}"><i class="fas fa-times me-1"></i>{{ __("Close Card") }}</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    $(".active-deactive-btn").click(function(){
        var actionRoute =  "{{ setRoute('user.cardyfie.virtual.card.make.default.or.remove') }}";
    ...
    $(".close-card").click(function(){
        var actionRoute =  "{{ setRoute('user.cardyfie.virtual.card.close') }}";
        var target = $(this).data('id');
        var btnText = $(this).text();
        var btnTextTitle ="{{ __('Close') }}";
        var message     = `Are you sure to  <strong>${btnTextTitle}</strong> this card?`;
        openAlertModal(actionRoute,target,message,btnText,"POST");
    });
</script>
@endpush
