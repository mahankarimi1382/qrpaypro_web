    ...
    <div class="body-wrapper payment-body-wrapper">
        <div class="custom-card payment-card">
            <div class="payment-type-wrapper">
                ...
                <form action="{{ setRoute('user.payment-link.update') }}" class="payment-form" method="POST"
                            ...
                            <div class="col-xl-12">
                                <div class="form-group">
                                    <select class="nice-select payment-select" name="type">
                                        <option
                                            {{ $payment_link->type == payment_gateway_const()::LINK_TYPE_PAY ? 'selected' : '' }}
                                            value="{{ payment_gateway_const()::LINK_TYPE_PAY }}" selected>
                                            {{ __('Customers Choose What To Pay') }}</option>
                                        <option
                                            {{ $payment_link->type == payment_gateway_const()::LINK_TYPE_SUB ? 'selected' : '' }}
                                            value="{{ payment_gateway_const()::LINK_TYPE_SUB }}">
                                            {{ __('Products Or Subscriptions') }}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="payment-box payment-box-area" id="pay-view">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="select2-auto-tokenize currency_link" name="currency">
                                            ...
                                            @foreach ($currency_data as $item)
                                                <option value="{{ $item->code }}" data-country="{{ $item->name }}"
                                                    data-currency_name="{{ $item->name }}"
                                                    data-code="{{ $item->code }}"
                                                    data-currency_symbol="{{ $item->symbol }}"
                                                    {{ $payment_link->currency == $item->code ? 'selected' : '' }}>
                        ...
                        <div class="payment-check-group">
                            <div class="custom-check-group">
                                <input type="checkbox" id="level-1" class="dependency-checkbox"
                                    data-target="payment-check-form" name="limit">
                                    ...
                                    <div class="col-xl-6">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" class="form--control number-input" placeholder="0.3"
                                                    min="0.1" name="min_amount"
                                                    value="{{ old('min_amount', $payment_link->min_amount ? get_amount($payment_link->min_amount, null, $payment_link->precision) : '') }}">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-xl-6">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" class="form--control number-input"
                                                    placeholder="10,000" min="0.2" name="max_amount"
                                                    value="{{ old('max_amount', $payment_link->max_amount ? get_amount($payment_link->max_amount, null, $payment_link->precision) : '') }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-xl-12 form-group">
                                    ...
                                    <select class="select2-auto-tokenize currency_link_sub" name="sub_currency">
                                        ...
                                        @foreach ($currency_data as $item)
                                            <option value="{{ $item->code }}" data-country="{{ $item->name }}"
                                                data-currency_name="{{ $item->name }}" data-code="{{ $item->code }}"
                                                data-currency_symbol="{{ $item->symbol }}"
                                                {{ $payment_link->currency == $item->code ? 'selected' : '' }}>
                                                {{ $item->code . ' (' . $item->name . ')' }}</option>
                                        @endforeach
                                ...
                                <div class="col-xl-6 col-lg-6 form-group">
                                    ...
                                    <div class="input-group">
                                        <div class="input-group-text prepend currency_link_symbol">
                                            {{ $payment_link->currency_symbol }}</div>
                                        <input type="text" class="form--control sub_price number-input"
                                            placeholder="0.00" name="price"
                                            value="{{ old('sub_price', get_amount(@$payment_link->price, null, $payment_link->precision)) }}">
                                    </div>
                                </div>
                                <div class="col-xl-6 col-lg-6 form-group">
                                    <label>{{ __('quantity') }}*</label>
                                    <input type="text" class="form--control qty_change number-input" min="1"
                                        name="qty" value="{{ old('qty', $payment_link->qty ?? '1') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    @if ($basic_settings->user_pin_verification == true)
                        <a href="javascript:void(0)" class="btn--base w-100 btn-loading" data-bs-toggle="modal"
                            data-bs-target="#checkPin">{{ __('update Link') }}</a>
                        ...
                        <nav>
                            <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                <button class="nav-link" id="mobile-tab" data-bs-toggle="tab" data-bs-target="#mobile"
                                    type="button" role="tab" aria-controls="mobile" aria-selected="false"><i
                                        class="las la-mobile-alt"></i></button>
                                <button class="nav-link active" id="web-tab" data-bs-toggle="tab"
                                    data-bs-target="#web" type="button" role="tab" aria-controls="web"
                                    aria-selected="true"><i class="las la-tv"></i></button>
                            </div>
                        </nav>
                    </div>
                </div>
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade" id="mobile" role="tabpanel" aria-labelledby="mobile-tab">
                        <div class="payment-preview-mockup">
                            <img src="{{ get_image('frontend/') }}/images/element/mockup.png" alt="element">
                                    ...
                                    <form class="payment-preview-box-left-form">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <div class="input-group-text prepend currency_link_symbol">
                                                    {{ $payment_link->currency_symbol }}</div>
                                                <input type="text" class="form--control paylink_amount"
                                                    value="{{ get_amount($payment_link->price * $payment_link->qty, null, $payment_link->precision) }}"
                                                    min="0.1" readonly>
                                            </div>
                                            <span
                                                class="limit-show">{{ get_amount($payment_link->min_amount, null, $payment_link->precision) }}
                                                {{ $payment_link->currency }} -
                                                {{ get_amount($payment_link->max_amount, null, $payment_link->precision) }}
                                                {{ $payment_link->currency }}</span>
                                        </div>
                                    </form>
                                    ...
                                    <form class="payment-preview-box-right-form">
                                        <div class="row">
                                            ...
                                            <div class="col-xl-12">
                                                ...
                                                <input type="text" class="form--control number-input"
                                                    placeholder="1234 1234 1234 1234" readonly>
                                                <div class="row mt-10">
                                            ...
                                            <div class="col-xl-12 form-group">
                                                <div class="preview-check-group">
                                                    <div class="custom-check-group">
                                                        ...
                                                        <label
                                                            for="level-2">{{ __('Securely save my information  for 1-click checkout') }}
                                                            <span>{{ __('Pay faster on') }}
                                                                {{ Auth::user()->address->company_name ?? '' }}
                                                                {{ __('and everywhere Link is accepted') }}</span></label>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-xl-12 form-group pt-10">
                                                <button type="submit" class="btn--base disabled w-100 btn-loading"
                                                    disabled>{{ __('Pay') }}</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                                ...
                                <form class="payment-preview-box-left-form">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <div class="input-group-text prepend currency_link_symbol">
                                                {{ $payment_link->currency_symbol }}</div>
                                            <input type="text" class="form--control paylink_amount" placeholder="0.00"
                                                min="0.1" readonly
                                                value="{{ get_amount($payment_link->price * $payment_link->qty, null, $payment_link->precision) }}">
                                        </div>
                                        <span class="limit-show">{{ __('limit') }}:
                                            {{ get_amount($payment_link->min_amount, null, $payment_link->precision) }}
                                            {{ $payment_link->currency }} -
                                            {{ get_amount($payment_link->max_amount, null, $payment_link->precision) }}
                                            {{ $payment_link->currency }}</span>
                                    </div>
                                        ...
                                        <div class="col-xl-12 form-group">
                                            <div class="preview-check-group">
                                                <div class="custom-check-group">
                                                    ...
                                                    <label
                                                        for="level-3">{{ __('Securely save my information  for 1-click checkout') }}
                                                        <span>{{ __('Pay faster on') }}
                                                            {{ Auth::user()->address->company_name ?? '' }} and everywhere
                                                            Link is accepted</span></label>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-xl-12 form-group pt-10">
                                            <button type="submit" class="btn--base disabled  w-100"
                                                disabled>{{ __('Pay') }}</button>
                                        </div>
                                    </div>
                                </form>
    ...
    <script>
        ...
        function limitShowPreveiw(currency_code) {
            $(document).on('keyup', 'input[name="min_amount"], input[name="max_amount"]', function() {
                ...
                $('.limit-show').text(parseFloat(min_limit).toFixed(2) + ' ' + currency_code + ' - ' + parseFloat(
                    max_limit).toFixed(2) + ' ' + currency_code + '');
            });
        }
    </script>
@endpush
