    ...
    <div class="body-wrapper payment-body-wrapper">
        <div class="custom-card payment-card">
            <div class="payment-type-wrapper">
                ...
                <form action="{{ setRoute('merchant.payment-link.store') }}" class="payment-form" method="POST"
                            ...
                            <div class="col-xl-12">
                                <div class="form-group">
                                    <select class="nice-select payment-select" name="type">
                                        <option value="{{ payment_gateway_const()::LINK_TYPE_PAY }}" selected>
                                            {{ __('Customers Choose What To Pay') }}</option>
                                        <option value="{{ payment_gateway_const()::LINK_TYPE_SUB }}">
                                ...
                                <div class="col-xl-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form--control link_title" name="title"
                                            placeholder="Name of cause or service" value="{{ old('title') }}">
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="select2-auto-tokenize currency_link" name="currency">
                                            ...
                                            @foreach ($currency_data as $item)
                                                <option value="{{ $item->code }}" data-country="{{ $item->country }}"
                                                    data-currency_name="{{ $item->name }}"
                                                    data-currency_code="{{ $item->code }}"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" class="form--control number-input"
                                                    placeholder="10,000" name="max_amount" value="{{ old('max_amount') }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-xl-12 form-group">
                                    ...
                                    <input type="text" class="form--control link_title" name="sub_title"
                                        placeholder="Collecting Payment Platform" value="{{ old('sub_title') }}">
                                </div>
                                <div class="col-xl-12 form-group">
                                    <label>{{ __('currency') }}</label>
                                ...
                                <div class="col-xl-6 col-lg-6 form-group">
                                    ...
                                    <div class="input-group">
                                        <div class="input-group-text prepend currency_link_symbol">$</div>
                                        <input type="text" class="form--control sub_price number-input" value=""
                                            placeholder="0.00" name="price">
                                    </div>
                                </div>
                                <div class="col-xl-6 col-lg-6 form-group">
                                    <label>{{ __('quantity') }}*</label>
                                    <input type="text" class="form--control qty_change number-input" value="1"
                                        min="1" name="qty">
                                </div>
                            </div>
                        </div>
                    ...
                    @if ($basic_settings->merchant_pin_verification == true)
                        <a href="javascript:void(0)" class="btn--base w-100 btn-loading" data-bs-toggle="modal"
                            data-bs-target="#checkPin">{{ __('Create New Link') }}</a>
                        @include('merchant.components.modal.pin-check')
                    @else
                        <button type="submit"
                            class="btn--base mt-20 w-100 btn-loading">{{ __('Create New Link') }}</button>
                    @endif
                </form>
            </div>
                                ...
                                <div class="payment-preview-box-left">
                                    <form class="payment-preview-box-left-form">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <div class="input-group-text prepend currency_link_symbol">$</div>
                                                <input type="text" class="form--control paylink_amount" value="0.00"
                                                    min="0.1" readonly>
                                            </div>
                                            ...
                                            <div class="col-xl-12 form-group">
                                                <div class="input-group two">
                                                    <div class="input-group-text prepend">
                                                        <img src="{{ get_image('frontend/images/icon/credit-card.png') }}"
                                                            alt="">
                                                    </div>
                                                    <input type="text" class="form--control"
                                                        placeholder={{ __('card Number') }} name="card_name"
                                                        value="{{ old('card_name') }}" readonly>
                                                    <div class="input-group-text append">{{ __('MM / YY / CVC') }}</div>
                                                </div>
                                            </div>
                                            <div class="col-xl-12 form-group">
                                                <div class="preview-secure-group">
                                                    <img src="{{ get_image('frontend/images/icon/100-percent.png') }}"
                                                        alt="">
                                                    <p>{{ __('Securely save my information  for 1-click checkout') }}
                                                        <span>{{ __('Pay faster on') }}
                                                            {{ Auth::user()->address->company_name ?? '' }}
                                                            {{ __('and everywhere Link is accepted') }}</span>
                                                    </p>
                                                </div>
                                            </div>
...
@push('script')
    ...
    <script>
        ...
        $(document).on('keyup', 'input[name="min_amount"], input[name="max_amount"]', function() {
            ...
            previewShow('.currency_link', min_limit = min_limit == '' ? 0 : min_limit, max_limit = max_limit == '' ?
                0 : max_limit);
        });
    </script>
@endpush
